...
Page({
    ...
    async safeLoadTemplate() {
        try {
            const template = await (0, db_1.getWeeklyTemplate)();
            this.setData({
    ...
    hasUsableCardRects() {
        return (this._cardRects.length === this.data.sections.length &&
            this._cardRects.every((rect) => typeof rect.top === "number" &&
    ...
    async handleSave() {
        try {
            await (0, db_1.saveWeeklyTemplate)(this.data.sections);
            this.setData({
                hasUnsavedChanges: false
            });
            wx.showToast({
